pipeline {
   ...
   stages {
      ...
      stage ('Build') {
          ...
          post {
              failure {
                  mail to: 'mep_eisen@web.de', subject: 'build of Sudoku failed', body: 'build of Sudoku failed'
              }
          }
      }
      ...
      stage ('Upload') {
        steps {
            ...
            sh "/srv/hudson/upload_game.sh ${env.BUILDTYPE} target 1 Sudoku MGSudoku-Plugin"
      	}
      }
      ...
      stage ('Deploy') {
          ...
          post {
              failure {
                  mail to: 'mep_eisen@web.de', subject: 'deploy of Sudoku failed', body: 'deploy of Sudoku failed'
              }
          }
      }
   }
